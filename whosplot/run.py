...
from whosplot.minkowski import Minkowski
from whosplot.superquadrics import SuperQuadrics
from whosplot.primitives2d import Primitives2D
...
class Run(Abstract):
    ...
    def __init__(self):
        ...
        self.minkowski = Minkowski(self)
        self.superquadrics = SuperQuadrics(self)
        self.primitives2d = Primitives2D(self)
        print_header()

    def __getattr__(self, name):
        for mod in (
            getattr(self, "minkowski", None),
            getattr(self, "superquadrics", None),
        ):
    ...
    def __alphabet_letter(self, index: int) -> str:
        """Return a lowercase alphabetic serial for the given figure index."""
        if not alphabet:
            raise ValueError("Alphabet list is empty.")
    ...
    def text(
        ...
        for fig_num in range(self.__figure_number):
            self.axs[int(fig_num // self.__cols), int(fig_num % self.__cols)].annotate(
                text[fig_num],
                xy=xypos,
    ...
    def __figure_serial(self, fig_num, use_tex):
        letter = self.__alphabet_letter(fig_num)
        if use_tex:
            serial = rf"\textbf{{({letter})}}"
        else:
            serial = f"({letter})"
        ax = self.axs[int(fig_num // self.__cols), int(fig_num % self.__cols)]
        ax.annotate(
            ...
            size=self.__scale_per_fig[fig_num] * 20,
        )

    def __cal_color_map_array(
        self, color_gradient: list, func="Euclidean_distance", sort_value=None
    ):
        color_array = {}
        ...
        for fig_num in range(self.__figure_number):
            ...
            if self.__figure_kind[fig_num] == "mix":
                ...
                line_index_list = [
                    ...
                    if int((i * 2) + 1) not in scatter_index_
                ]
                ...
                for scatter_num in range(len(scatter_index_list)):
                    index_ = int(
                        scatter_num
                        * color_map_array_len
                        / max((len(scatter_index_list) - 1), 1)
                    )
                    color_array[fig_num, scatter_index_list[scatter_num]] = (
                        self.__color_map_array[fig_num][index_]
                    )
                for line_num in range(len(line_index_list)):
                    index_ = int(
                        line_num
                        * color_map_array_len
                        / max((len(line_index_list) - 1), 1)
                    )
                    color_array[fig_num, line_index_list[line_num]] = (
                        self.__color_map_array[fig_num][index_]
                    )
                ...
                for line_num in range(legend_len):
                    if (
                        color_gradient[fig_num][line_num]
                        and func == "Euclidean_distance"
                    ):
                        data_mmnorm = calculator(
                            method="mmnorm", data=self.__data[fig_num], axis=1
                        )
                        x = data_mmnorm[:, 0]
                        y = data_mmnorm[:, int(line_num * 2 + 1)]
                        dist_ = calculator(
                            method="dist", data=np.array([x, y]).T, axis=0, power=2
                        )
                        ...
                        index_ = int(
                            line_num * color_map_array_len / max((legend_len - 1), 1)
                        )
                        color_array[fig_num, line_num] = self.__color_map_array[
                            fig_num
                        ][index_]
        ...
        for fig_num in range(self.__figure_number):
            ...
            for line_num in range(legend_len):
                ...
                if self.__figure_number > 1:
                    self.__figure_serial(
                        fig_num, use_tex=self.plt.rcParams["text.usetex"]
                    )
    ...
    def color_gradient_two_d_subplots(
        self, func="Euclidean_distance", sort_value=None, data_range=None
    ):
        """Create 2D subplots with color gradients."""
    ...
    def pv_screenshot_arrange(
        ...
        if len(img_path) != self.__figure_number:
            raise ValueError(
                "The number of images does not match the number of subplots."
            )
        ...
        if label_type == 1:
            ...
            for fig_num in range(self.__figure_number):
                ...
                if tick_type[fig_num] == "log":
                    ...
                    ticks = np.logspace(
                        np.log10(min_), np.log10(max_), tick_num[fig_num]
                    )
                else:
                ...
                self.axs[int(fig_num // self.__cols), int(fig_num % self.__cols)].axis(
                    "off"
                )
                ...
                if self.__figure_number > 1:
                    serial = rf"\textbf{{({self.__alphabet_letter(fig_num)})}}"
                    self.axs[
                        int(fig_num // self.__cols), int(fig_num % self.__cols)
                    ].annotate(
                        serial,
                ...
                self.__colorbar(
                    fig_num, data_range[fig_num], label_name[fig_num], "horizontal"
                )
            ...
            fig_ratio = [
                figop_remove_margin(img_path[fig_num]).size[1]
                / figop_remove_margin(img_path[fig_num]).size[0]
                for fig_num in range(self.__figure_number)
            ]
            ...
            cbar_ax = self.fig.add_axes(
                [cbar_left / fig_width, 0.05, cbar_width / fig_width, 0.02]
            ...
            if len(tick_type) != len(data_range) and len(tick_type) != 100:
                raise ValueError(
                    "The number of tick types does not match the number of colorbars."
                )
            ...
            for label_num in range(num_colorbars):
                ...
                if tick_type_list[label_num] == "log":
                    ...
                    ticks = np.logspace(
                        np.log10(min_), np.log10(max_), tick_num[label_num]
                    )
                else:
                    norm = Normalize(vmin=min_, vmax=max_)
                ...
                sm = self.plt.cm.ScalarMappable(
                    norm=norm, cmap=self.__color_map[label_num]
                )
                cbar = self.fig.colorbar(
                    sm,
                    cax=cbar_ax,
    ...
    def draw_superquadrics(self, items):
        return self.superquadrics.draw_superquadrics(items)

    def plot_minkowski_difference2d(self, *args, **kwargs):
        """Proxy to :meth:`Minkowski.plot_minkowski_difference2d`."""

        return self.minkowski.plot_minkowski_difference2d(*args, **kwargs)

    def plot_superellipses2d(self, *args, **kwargs):
        """Proxy to :meth:`Minkowski.plot_superellipses2d`."""

        return self.minkowski.plot_superellipses2d(*args, **kwargs)

    def plot_superellipse2d(self, *args, **kwargs):
        """Proxy to :meth:`Minkowski.plot_superellipse2d`."""

        return self.minkowski.plot_superellipse2d(*args, **kwargs)

    def plot_minkowski_sum2d(self, *args, **kwargs):
        """Proxy to :meth:`Minkowski.plot_minkowski_sum2d`."""

        return self.minkowski.plot_minkowski_sum2d(*args, **kwargs)

    def draw_vector2d(self, *args, **kwargs):
        """Proxy to :meth:`Primitives2D.draw_vector2d`."""
        return self.primitives2d.draw_vector2d(*args, **kwargs)

    def draw_wall2d(self, *args, **kwargs):
        """Proxy to :meth:`Primitives2D.draw_wall2d`."""
        return self.primitives2d.draw_wall2d(*args, **kwargs)
