...
class CsvReader(Abstract):
    ...
    def __set_attributes(self, key):
        ...
        if if_none(key):
            ...
            self.__Parameter.__setattr__(
                "file_location", file_location.replace(file_name, key)
            )
    ...
    def __insert_whos_plot_helper(list_, num):
        for num_ in range(0, 100, 2):
            if (
                num_ < len(list_[num])
                and "whos_plot" not in list_[num][num_]
                and num_ != 0
            ):
    ...
    def __numpy_save_data(cls, file_name: str, data, header: list) -> int:
        ...
        """
        np.savetxt(
            fname="{}_standardized.csv".format(file_name),
            X=data,
            fmt="%.18f",
            delimiter=",",
            header=",".join(header),
    ...
    @classmethod
    def __insert_x_data(
        cls, data: np.ndarray, legend: list, header: list
    ) -> np.ndarray:
        """
        ...
        if isinstance(data, np.ndarray):
            for num in range(len(label_position)):
                if num == 0:
                    ...
                    data_list.append(
                        data[:, label_position[num] : label_position[num + 1]]
                    )
    ...
    def __get_label_position(self) -> list:
        ...
        return [
            count
            for count in range(len(self.header))
            if len(self.header[count].split("::")) > 1
        ]
    ...
    def __set_label_list(self):
        ...
        for num in range(len(self.label)):
            if self.label_len_list[num] != len(self.__limited_label[num]) / 2:
                raise ValueError(
                    "The number of labels is not equal to the number of limited labels."
                )
    ...
    def __standardize_data_array(self) -> list:
        ...
        data_list = [
            self.__insert_x_data(data_list_[num], legend[num], header[num])
...
class RegularExpression:
    ...
    def Bn(cls, text, root):
        look_up = re.findall(
            r"nucleationSiteModel.*?Bn.*?([\d+/.*]+);", text, re.DOTALL
        )
        if len(look_up) == 0:
    ...
    def Cn(cls, text, root):
        look_up = re.findall(
            r"nucleationSiteModel.*?Cn.*?([\d+/.*]+);", text, re.DOTALL
        )
        if len(look_up) == 0:
...
class OfPostProcess:
    def __init__(self, folder_path: str, dict_key: list = None, drop_out: int = 1):
        ...
        with open(file_path, "r", encoding="utf-8-sig") as f:
            ...
            for line in reader[::drop_out]:
                if not line.startswith("#"):
                    ...
                    velocity_line = np.array(
                        [np.array(Uxyz.split(" ")).astype(float) for Uxyz in re_find]
                    ).flatten()
    ...
    @classmethod
    def __dict_in_list_to_array(
        cls, dict_in_list: list, row: int, col: int, lambda_func: callable
    ):
        """
        ...
        value_array = np.array([list(x.values())[0] for x in dict_in_list]).reshape(
            (row, col)
        )
        key_array = np.array([list(x.keys())[0] for x in dict_in_list]).reshape(
            (row, col)
        )
        return key_array, value_array

    def surfaceFieldValue_to_csv(self):
        log_file_path = self.__get_surfaceFieldValue_path(
        ...
        for log_file in log_file_path:
            ...
            for file in self.file_name:
                ...
                if flag:
                    self.numpy_save_data(
                        f"./{case_num}-{flag[0]}.csv", data_load, header=header
                    )
                    break

    def residuals_to_csv(self, auto_save=True):
        dic = {}
        residuals_file_path = self.__get_surfaceFieldValue_path(
            self.folder_path, file_list=["residuals"]
        )
        for residuals_file in residuals_file_path:
            ...
            if auto_save:
                self.numpy_save_data(
                    f"./case{case_num}-residuals.csv", data_load, header=header
                )
    ...
    def probes_vector_to_csv(self, auto_save=True):
        ...
        for probesU_file in probesU_file_path:
            data_load = self.__probes_vector_to_array(
                probesU_file, drop_out=self.drop_out
            )
            ...
            if auto_save:
                self.numpy_save_data(
                    f"./case{case_num}-probesU.csv", data_load, header=header
                )
            dic[f"case{case_num}"] = data_load
        return dic
    ...
    def surfaceFieldValue_time_average_scalar(self, auto_save=True):
        ...
        for log_file in log_file_path:
            ...
            ave_scalar = calculator(
                mathod="of_time_average", data=data_limited, axis=1
            )[0][1]
            case_num = re.findall(r".*?\\case(\d+)", log_file)[0]
            for file in self.file_name:
                flag = re.findall(r"\\({})\\".format(file), log_file)
        ...
        save_value = np.array([list(x.values())[0] for x in ave_scalar_list]).reshape(
            (row, col)
        )
        save_key = np.array([list(x.keys())[0] for x in ave_scalar_list]).reshape(
            (row, col)
        )
    ...
    def probes_vector_U_time_average(self, auto_save=True):
        ...
        for probesU_file in probesU_file_path:
            ...
            if auto_save:
                header = ["Time_mean"] + [
                    f"UxMean{i}, UyMean{i}, UzMean{i}" for i in range(0, last_num)
                ]
                self.numpy_save_data(
                    f"./case{case_num}-ave_velocity.csv", ave_velocity, header=header
                )
                ...
                for file_name, parameters in file_name_dict.items():
                    if file_name in cpp_file:
                        ...
                        for key in parameter_key:
                            dic[f"{case_num}-{key}"] = float(
                                parameters[key](reader, cpp_file)
                            )

            except IndexError:
                print(
                    f"case num not found in {cpp_file} \nand this folder will be ignored"
                )
        dic_ = {}
