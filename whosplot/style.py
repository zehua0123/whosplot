...
class Style(Abstract):
    ...
    def __set_attributes(self):
        ...
        for attr in parameter_attributes:
            setattr(
                self,
                f"_{self.__class__.__name__}__{attr}",
                getattr(self.__Parameter, attr),
            )
    ...
    def __create_figure(self):
        ...
        for num in range(self.__figure_number):
            if self.__label_len_list[num] == 3:
                self.ax0 = self.axs[
                    int(num // self.__cols), int(num % self.__cols)
                ].twinx()

        self.__REF_WIDTH_IN = 8.0
        self.__REF_AXES_LW = 2.0
        self.__REF_LEGEND_FONTSIZE = 14
        self.__REF_LABELSIZE = 32
        self.__REF_LABELPAD = 8
        self.__REF_TICKLABEL = 32
        ...
        self.scale_per_fig = [
            max(self.__width / self.__REF_WIDTH_IN, 0.1)
        ] * self.__figure_number
    ...
    def __set_axis(self):
        ...
        self.plt.rcParams["axes.grid.which"] = (
            "major"  # grid lines at {major, minor, both} ticks
        )
        self.plt.rcParams["axes.titlelocation"] = (
        ...
        self.plt.rcParams["axes.formatter.use_mathtext"] = (
            False  # When True, use mathtext for scientific notation.
        )
        self.plt.rcParams["axes.formatter.min_exponent"] = (
        ...
        self.plt.rcParams["axes.xmargin"] = 0.0  # x margin.  See `axes.Axes.margins`
        self.plt.rcParams["axes.ymargin"] = 0.05  # y margin.  See `axes.Axes.margins`
        self.plt.rcParams["axes.zmargin"] = 0.05  # z margin.  See `axes.Axes.margins`
        ...
        self.plt.rcParams["xaxis.labellocation"] = (
            "center"  # alignment of the xaxis label: {left, right, center}
        )
        self.plt.rcParams["yaxis.labellocation"] = (
    ...
    def __set_tick(self):
        ...
        self.plt.rcParams["xtick.major.pad"] = (
            6  # distance to major tick label in points
        )
        self.plt.rcParams["xtick.minor.pad"] = (
            6.1  # distance to the minor tick label in points
        )
        self.plt.rcParams["xtick.color"] = "black"  # color of the ticks
        self.plt.rcParams["xtick.labelcolor"] = (
        ...
        self.plt.rcParams["xtick.minor.visible"] = (
        ...
        self.plt.rcParams["ytick.major.pad"] = (
            6  # distance to major tick label in points
        )
        self.plt.rcParams["ytick.minor.pad"] = (
            6.1  # distance to the minor tick label in points
        )
        self.plt.rcParams["ytick.color"] = "black"  # color of the ticks
        self.plt.rcParams["ytick.labelcolor"] = (
        ...
        self.plt.rcParams["ytick.minor.visible"] = (
    ...
    def __set_figure(self):
        ...
        self.plt.rcParams["figure.raise_window"] = (
            False  # Raise the GUI window to front when show() is called.
        )

        """The figure subplot parameters.  All dimensions are a fraction of the figure width and height."""
        self.plt.rcParams["figure.subplot.left"] = (
            0.125  # the left side of the subplots of the figure
        )
        self.plt.rcParams["figure.subplot.right"] = (
            0.9  # the right side of the subplots of the figure
        )
        self.plt.rcParams["figure.subplot.bottom"] = (
            0.11  # the bottom of the subplots of the figure
        )
        self.plt.rcParams["figure.subplot.top"] = (
        ...
        self.plt.rcParams["figure.constrained_layout.h_pad"] = (
            0.04167  # Padding around axes objects. Float representing
        )
        self.plt.rcParams["figure.constrained_layout.w_pad"] = (
            0.04167  # inches. Default is 3/72 inches (3 points)
        )
        # self.plt.rcParams['figure.constrained_layout.w_pad'] = 0.6  # inches. Default is 3/72 inches (3 points)
        self.plt.rcParams["figure.constrained_layout.hspace"] = (
            0.02  # Space between subplot groups. Float representing
        )
        self.plt.rcParams["figure.constrained_layout.wspace"] = (
            0.02  # a fraction of the subplot widths being separated.
        )
    ...
    def __set_font(self):
        ...
        self.plt.rcParams["font.cursive"] = [
            "Apple Chancery",
            "Textile",
            "Zapf Chancery",
            "Sand",
        ...
        self.plt.rcParams["font.fantasy"] = [
            "Chicago",
            "Charcoal",
            "Impact",
            "Western",
        ...
        self.plt.rcParams["font.monospace"] = [
            "DejaVu Sans Mono",
    ...
    def __set_latex(self):
        ...
        self.plt.rcParams["text.latex.preamble"] = r"\usepackage{amsmath, bm}"
        ...
        self.plt.rcParams["mathtext.fontset"] = (
            "cm"  # The font set to use for math text.
        )
    ...
    def __set_label_type(self):
        ...
        for num in range(self.__figure_number):
            ...
            if x_label_type[num] is not None and x_label_type[num] != "linear":
                ...
                ax.set_xscale(x_label_type[num], base=2)
                ax.xaxis.set_major_locator(plt.LogLocator(base=2))
                # Format x-axis tick labels
                if self.__x_ticklabel_format[num] != "plain":
                    ax.xaxis.set_major_formatter(
                        plt.FuncFormatter(
                            lambda val, pos: format_func(
                                val, pos, self.__x_ticklabel_format[num]
                            )
                        )
                    )
            if y_label_type[num] is not None and y_label_type[num] != "linear":
                ...
                if self.__y_ticklabel_format[num] != "plain":
                    ax.yaxis.set_major_formatter(
                        plt.FuncFormatter(
                            lambda val, pos: format_func(
                                val, pos, self.__y_ticklabel_format[num]
                            )
                        )
                    )
    ...
    def __set_ticklabel_format(self):
        ...
        for num in range(self.__figure_number):
            if (
                self.__x_ticklabel_format is None
                or self.__x_ticklabel_format[num] == "plain"
            ):
            ...
            if (
                self.__y_ticklabel_format is None
                or self.__y_ticklabel_format[num] == "plain"
            ):
    ...
    def __set_title(self):
        ...
        if self.__title:
            for num in range(self.__figure_number):
                self.axs[int(num // self.__cols), int(num % self.__cols)].set_title(
                    self.__title, fontdict={"size": 26, "weight": "bold"}
                )
    ...
    def __set_label_limitation(self):
        ...
        for fignum in range(self.__figure_number):
            if (
                len(self.__limited_label[fignum]) == 4
                or len(self.__limited_label[fignum]) == 6
            ):
                if (
                    self.__limited_label[fignum][0] != 0
                    or self.__limited_label[fignum][1] != 0
                ):
                    self.axs[
                        int(fignum // self.__cols), int(fignum % self.__cols)
                    ].set_xlim(
                        self.__limited_label[fignum][0], self.__limited_label[fignum][1]
                    )

                if (
                    self.__limited_label[fignum][2] != 0
                    or self.__limited_label[fignum][3] != 0
                ):
                    self.axs[
                        int(fignum // self.__cols), int(fignum % self.__cols)
                    ].set_ylim(
                        self.__limited_label[fignum][2], self.__limited_label[fignum][3]
                    )

            if len(self.__limited_label[fignum]) == 6:
                if (
                    self.__limited_label[fignum][4] != 0
                    and self.__limited_label[fignum][5] != 0
                ):
                    self.axs_dict[fignum].set_ylim(
                        self.__limited_label[fignum][4], self.__limited_label[fignum][5]
                    )
            ...
            if (
                len(self.__limited_label[fignum]) != 4
                and len(self.__limited_label[fignum]) != 6
            ):
        ...
        for num in range(self.__figure_number):
            ...
            if kwargs.get("extra_legend") is not None:
                ...
                legend_new = self.__legend[num] + [
                    item
                    for sublist in [
                        arg_list[i : i + 2] for i in range(0, len(arg_list), 4)
                    ]
                    for item in sublist
                ]
                ...
                if self.__legend_len_list[num] == 1:
                    return (
                        0  # Assuming the function should terminate early in this case
                    )
            ...
            if self.__label_len_list[num] != 3:
                labels = (
                    self.axs[int(num // self.__cols), int(num % self.__cols)]
                    .legend(
                        legend_items,
                        loc=self.__legend_location[num],
                        frameon=False,
                        fontsize=self.__REF_LEGEND_FONTSIZE * self.scale_per_fig[num],
                        handlelength=2.5,
                    )
                    .get_texts()
                )
        ...
        for num in range(self.__figure_number):
            if self.__label_len_list[num] == 2 or self.__label_len_list[num] == 3:
                self.axs[int(num // self.__cols), int(num % self.__cols)].set_xlabel(
                    self.__label[num][0], fontdict={"weight": "bold"}
                )
                self.axs[int(num // self.__cols), int(num % self.__cols)].set_ylabel(
                    self.__label[num][1], fontdict={"weight": "bold"}
                )
    ...
    def __set_marker(self):
        ...
        for num in range(self.__figure_number):
            if self.__figure_kind[num] == "scatter" and line_with_marker[num] is True:
                ...
                self.__figure_kind[num] == "scatter"
                and type(line_with_marker[num]) == list
            ):
                self.marker.append(line_with_marker[num])
            elif (
                self.__figure_kind[num] == "plot"
                and type(line_with_marker[num]) == list
            ):
                self.marker.append(line_with_marker[num])
            elif (
                self.__figure_kind[num] == "plot"
                and line_with_marker[num] is True
                and multi_line_style[num] is False
            ):
                self.marker.append(filled_markers)
            ...
            if multi_line_style[num] is True:
                ...
                for i in range(len(multi_line_style[num])):
                    if multi_line_style[num][i] not in linestyle_dict:
                        ...
                        multi_line_style_.append(
                            linestyle_dict[multi_line_style[num][i]]
                        )
