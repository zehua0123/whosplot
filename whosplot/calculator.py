...
def __var(data: np.ndarray, axis: int):
    ...
    if axis == 1:
        ...
        return np.array([np.var(data[:, num]) for num in range(col_num)]).reshape(
            1, col_num
        )
    elif axis == 0:
        row_num, col_num = data.shape
        return np.array([np.var(data[num, :]) for num in range(row_num)]).reshape(
            row_num, 1
        )
...
def __std(data: np.ndarray, axis: int):
    ...
    if axis == 1:
        row_num, col_num = data.shape
        return np.array([np.std(data[:, num]) for num in range(col_num)]).reshape(
            1, col_num
        )
    elif axis == 0:
        row_num, col_num = data.shape
        return np.array([np.std(data[num, :]) for num in range(row_num)]).reshape(
            row_num, 1
        )
...
def __mean(data: np.ndarray, axis: int):
    ...
    if axis == 1:
        row_num, col_num = data.shape
        return np.array([np.mean(data[:, num]) for num in range(col_num)]).reshape(
            1, col_num
        )
    elif axis == 0:
        row_num, col_num = data.shape
        return np.array([np.mean(data[num, :]) for num in range(row_num)]).reshape(
            row_num, 1
        )
...
def __max(data: np.ndarray, axis: int):
    ...
    if axis == 1:
        row_num, col_num = data.shape
        return np.array([np.max(data[:, num]) for num in range(col_num)]).reshape(
            1, col_num
        )
    elif axis == 0:
        row_num, col_num = data.shape
        return np.array([np.max(data[num, :]) for num in range(row_num)]).reshape(
            row_num, 1
        )
...
def __min(data: np.ndarray, axis: int):
    ...
    if axis == 1:
        row_num, col_num = data.shape
        return np.array([np.min(data[:, num]) for num in range(col_num)]).reshape(
            1, col_num
        )
    elif axis == 0:
        row_num, col_num = data.shape
        return np.array([np.min(data[num, :]) for num in range(row_num)]).reshape(
            row_num, 1
        )
...
def __mmnorm(data: np.ndarray, axis: int):
    ...
    if axis == 1:
        ...
        return np.array(
            [
                (data[:, num] - min_[:, num]) / (max_[:, num] - min_[:, num])
...
def __znorm(data: np.ndarray, axis: int):
    ...
    if axis == 1:
        ...
        return np.array(
            [(data[:, num] - mean_[:, num]) / std_[:, num] for num in range(col_num)]
        ).T
    elif axis == 0:
        row_num, col_num = data.shape
        mean_ = __mean(data, axis)
        std_ = __std(data, axis)
        return np.array(
            [(data[num, :] - mean_[num, :]) / std_[num, :] for num in range(row_num)]
        )
...
def __dist(data: np.ndarray, axis: int, power: int):
    ...
    if axis == 1:
        ...
        return np.array(
            [
                np.power(np.sum(np.power(data[:, num], power)), 1 / power)
                for num in range(col_num)
            ]
        ).reshape(1, col_num)
    elif axis == 0:
...
def __of_time_average(data: np.ndarray, axis: int):
    ...
    if axis == 1:
        ...
        return np.array(
            [
                np.sum(data[:, num] * weight[:, 0]) / np.sum(weight[:, 0])
                for num in range(col_num)
            ]
        ).reshape(1, col_num)
